...
// ── Récupère un éventuel achat fait AVANT la création du compte ──
// (le client a payé sur Lemon Squeezy avec cet email, mais n'avait pas
// encore de compte au moment du paiement — la Cloud Function a stocké
// l'achat dans "pendingPurchases", on le réclame ici.)
function reclamerAchatsEnAttente(user) {
  const email = (user.email || "").toLowerCase().trim();
  const pendingRef = db.collection("pendingPurchases").doc(email);

  return pendingRef.get().then((snap) => {
    if (!snap.exists) return;
    const planners = snap.data().planners || [];
    if (!planners.length) return pendingRef.delete();

    return db.collection("users").doc(user.uid).set(
      { email, planners: firebase.firestore.FieldValue.arrayUnion(...planners) },
      { merge: true }
    ).then(() => pendingRef.delete());
  }).catch((error) => {
    console.error("Erreur lors de la réclamation des achats en attente :", error);
    alert("DEBUG — erreur pendingPurchases : " + error.code + " / " + error.message);
  });
}
...
function redirigerVersPlanner(user) {
  ...
  db.collection("users").doc(user.uid).get()
    .then((doc) => {
      ...
      if (valid.length >= 1) {
        // On passe toujours par la page de choix — même avec un seul planner,
        // il y apparaît en clair, cliquable.
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  authForm.addEventListener("submit", (e) => {
    ...
    if (isSignup) {
      ...
      auth.createUserWithEmailAndPassword(email, password)
        ...
        .then(() => {
          return reclamerAchatsEnAttente(createdUser);
        })
        .then(() => {
          setLoading(false, submitBtn, defaultLabel);
          alert("Compte créé ! Si tu avais déjà acheté un planner avec cet email, il est maintenant disponible dans ton espace.");
        })
        .catch((error) => {
          setLoading(false, submitBtn, defaultLabel);
      ...
      auth.signInWithEmailAndPassword(email, password)
        .then((userCredential) => {
          return reclamerAchatsEnAttente(userCredential.user).then(() => userCredential.user);
        })
        .then((user) => {
  ...
  auth.onAuthStateChanged((user) => {
    if (user && !document.body.classList.contains("signup-mode")) {
      reclamerAchatsEnAttente(user).then(() => redirigerVersPlanner(user));
    }
  });
});
